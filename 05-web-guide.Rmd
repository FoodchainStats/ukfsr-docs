...
**CSV files:** use the current standard GOV.UK nomenclature for viewing/downloading associated data CSV files, for the data preview, e.g.  
```
[View the data for this chart](https://assets.publishing.service.gov.uk/media/NODE/tX_s_nv_description_DATE.csv/preview)  
```
and for the data download, e.g.
```
[Download the data for this chart](https://assets.publishing.service.gov.uk/media/NODE/tX_s_nv_description_DATE.csv)
```
where X is theme number, s is section number, n is indicator number, v is indicator variant, e.g. a,b,c, etc., description is content descriptor, DATE is publication date, in the recommended format e.g. 01jan2024. It will be good practice to have placeholders for these two urls placed directly underneath the accessibility text (as described above); the onduty publisher will insert the unique 'nodes' for each graphic in the last step before sending a preview page to the team or submitting the report for 2i approval. 
